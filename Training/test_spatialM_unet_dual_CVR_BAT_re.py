...
if __name__ == "__main__":
    ...
    model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_dual_corr_beta0_spatialM_aal_Epoch_100_AdaB1_1e12_new_re.pt'
    model = UNet_dual(in_channels=169, n_classes=2, depth=5, batch_norm=True, padding=True, up_mode='upconv')
    ...
    with torch.no_grad():
        for X_ID, X, X_affine in test_loader:
            ...
            bat = np.squeeze(prediction[0, 2].cpu().clone().numpy())
    ...
    for subname in glob.glob('*/'):
        ...
        for subfile in os.listdir(os.path.join(test_dir, subname)):
            if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-9][0-9][0-9]_ML_Unet_dual_result_CVR.nii'):
            ...
            if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-9][0-9][0-9]_ML_Unet_dual_result_BAT.nii'):
                parts = subfile.split('_')
                ID = int(parts[-6]) - 1
        ...
        img_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_aal_Unet_dual_CVR.nii'))

        img_bat_output = nib.Nifti1Image(img_bat_data_3D, img_mean.affine)
        img_bat_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_bat_output, os.path.join(test_dir, subname, 'ML_result_spatialM_aal_Unet_dual_BAT.nii'))
