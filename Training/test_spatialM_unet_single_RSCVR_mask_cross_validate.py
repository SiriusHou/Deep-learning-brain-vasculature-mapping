        ...
        for subname in os.listdir(data_root):
            if os.path.isdir(os.path.join(data_root, subname)):
                for subfile in os.listdir(os.path.join(data_root, subname)):
                    if fnmatch.fnmatch(subfile, 'CVR_mean_temporal_f_clean_0[0-9][0-9].nii'):
...
if __name__ == "__main__":
    ...
    for pod in range(5):
        ...
        model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate/' + pod_test_name + '/Unet_single_RSCVR_corr_beta0_spatialM_ce_nm_Epoch_100_AdaB1_1e12_final2_mask_soft_4.pt'
        ...
        with torch.no_grad():
            for X_ID, X, X_affine in test_loader:
                ...
                nib.save(img_output, '_'.join([X_dir, 'ML_Unet_single_result_RSCVR.nii']))
        ...
        for subname in glob.glob('*/'):
            ...
            for subfile in os.listdir(os.path.join(test_dir, subname)):
                if fnmatch.fnmatch(subfile, 'CVR_mean_temporal_f_clean_[0-9][0-9][0-9]_ML_Unet_single_result_RSCVR.nii'):
            ...
            img_output.get_data_dtype() == np.dtype(np.int16)
            nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_ce_nm_Unet_single_RSCVR4_only.nii'))
