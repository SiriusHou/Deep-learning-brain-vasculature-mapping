...
import math
from corr_ChannelAttnunet_ssim_CBAM_rp_model import ChannelAttnUNet
    ...
    with torch.no_grad():
        for subname in glob.glob('*\\'):
            ...
            for X, X_m, y in testloader:
                ...
                #         plt.imshow(X[0][1])
                #         plt.show()
                h, scale_out, x_h, prediction = model(X, X_m)
                h_result[i, :] = h.cpu().clone().numpy()
                scale_out_result[:, :, i] = np.squeeze(scale_out.cpu().clone().numpy())
            ...
            scale_out_output = nib.Nifti1Image(scale_out_result, taimg.affine)
            scale_out_output.get_data_dtype() == np.dtype(np.int16)
            nib.save(scale_out_output, os.path.join(test_dir, subname, 'ML_result_WS_ChannelAttnUnet_avg3_ssim_spatialAtt1.nii'))
