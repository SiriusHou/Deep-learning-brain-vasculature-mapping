...
class CVRDataset(data.Dataset):
            ...
            if os.path.isdir(os.path.join(data_root, subname)):
                for subfile in os.listdir(os.path.join(data_root, subname)):
                    if fnmatch.fnmatch(subfile, 'bold_cere_[0]*'):
                        ...
                        self.mask_IDs.append(os.path.join(parts_prefix, 'mask', 'brainMask_' + parts_end))
    ...
    def __getitem__(self, index):
        ...
        mask = torch.Tensor(mask_image).type(torch.FloatTensor)
...
if __name__ == "__main__":
    ...
    for t in range(epochs):
        ...
        for i, (X, X_m, y, mask) in enumerate(train_loader):
            X = X.to(device)  # [N, C, H, W]
            X_m = X_m.to(device) # [N, 1, H, W]
            y = y.to(device)  # [N, H, W] with class indices (0, 1)
            mask = mask.to(device)

            h, ssim_info = model(X, X_m, mask)  # [N, H, W]
            ...
            for kk in range(h.shape[1]):
                ...
                ssim_index = ssim(y, X_layer, size_average=None, val_range=10)
            ...
            loss = criterion(h, ssim_index_all)

            # ssim_index_max = torch.argmax(ssim_index_all, 1)
            # loss = criterion_cc(h,  ssim_index_max)
            ...
            if (i + 1) % print_every == 0:
                ...
                model.eval()
                with torch.no_grad():
                    for X, X_m, y, mask in validation_loader:
                        X = X.to(device)  # [N, C, H, W]
                        X_m = X_m.to(device)  # [N, 1, H, W]
                        y = y.to(device)  # [N, H, W] with class indices (0, 1)
                        mask = mask.to(device)
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('d:\\xhou4\\ML_CVR\\smooth8\\ML_data\\train\\ChannelAttnUnet_avg3_ssim_ws_valid_loss.png', dpi=300)
    plt.show()

    torch.save(model.state_dict(), 'd:\\xhou4\\ML_CVR\\smooth8\\ML_data\\ChannelAttnUnet_avg3_ssim_WS_134sub.pt')
