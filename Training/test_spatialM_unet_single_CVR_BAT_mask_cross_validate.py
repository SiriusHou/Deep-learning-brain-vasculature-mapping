...
import math
from model_spatialM_unet_single_CVR_BAT import UNet_single
        ...
        for subname in os.listdir(data_root):
            if os.path.isdir(os.path.join(data_root, subname)):
                for subfile in os.listdir(os.path.join(data_root, subname)):
                    if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_single_0[0-9][0-9].nii'):
...
if __name__ == "__main__":
    ...
    for pod in range(5):
        ...
        model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate/' + pod_test_name + '/Unet_single_corr_beta0_spatialM_ce_nm_Epoch_100_AdaB1_1e12_final2_mask_soft_4.pt'
        model = UNet_single(in_channels=3, n_classes=1, depth=5, batch_norm=True, padding=True, up_mode='upconv')
        ...
        for subname in glob.glob('*/'):
            ...
            for subfile in os.listdir(os.path.join(test_dir, subname)):
                if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_single_[0-9][0-9][0-9]_ML_Unet_single_result_CVR.nii'):
                ...
                if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_single_[0-9][0-9][0-9]_ML_Unet_single_result_BAT.nii'):
                    parts = subfile.split('_')
                    ID = int(parts[-6]) - 1
