...
if __name__ == "__main__":
    ...
    model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_corr_gl_spatialM_aal_Epoch_200.pt'
    model = UNet(in_channels=167, n_classes=1, depth=5, batch_norm=False, padding=True, up_mode='upconv').to(device)
    ...
    with torch.no_grad():
        for X_ID, X, X_affine in test_loader:
            ...
            nib.save(img_output, '_'.join([X_dir, 'ML_Unet_result.nii']))
    ...
    for subname in glob.glob('*/'):
        ...
        for subfile in os.listdir(os.path.join(test_dir, subname)):
            if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-9][0-9][0-9]_ML_Unet_result.nii'):
        ...
        img_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_aal_Unet.nii'))
