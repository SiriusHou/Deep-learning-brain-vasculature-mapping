...
if __name__ == "__main__":
    ...
    for pod in range(5):
        ...
        model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate/' + pod_test_name + '/Unet_CVR_separate_wsupple_corr_beta0_spatialM_ce_nm_Epoch_100_AdaB1_1e12_mask_soft_4.pt'
        ...
        with torch.no_grad():
            for X_ID, X, X_affine in test_loader:
                ...
                cvr_output = nib.Nifti1Image(cvr, X_affine)
                cvr_output.get_data_dtype() == np.dtype(np.int16)
                nib.save(cvr_output, '_'.join([X_dir, 'ML_Unet_separate_wsupple_result_CVR.nii']))
        ...
        for subname in glob.glob('*/'):
            ...
            for subfile in os.listdir(os.path.join(test_dir, subname)):
                if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_[0-9][0-9][0-9]_ML_Unet_separate_wsupple_result_CVR.nii'):
                    parts = subfile.split('_')
                    ID = int(parts[-7]) - 1

                    sub_filepath = os.path.join(test_dir, subname, subfile)
                    img = nib.load(sub_filepath)
                    img_cvr_data = img.get_fdata()
                    img.uncache()

                    img_cvr_data_3D[:, :, ID] = img_cvr_data[2:-3, 1:-2]

            img_cvr_output = nib.Nifti1Image(img_cvr_data_3D, img_mean.affine)
            img_cvr_output.get_data_dtype() == np.dtype(np.int16)
            nib.save(img_cvr_output, os.path.join(test_dir, subname, 'ML_result_spatialM_ce_nm_Unet_wsupple_result_CVR4.nii'))
