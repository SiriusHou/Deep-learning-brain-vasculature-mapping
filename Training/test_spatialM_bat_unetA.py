        ...
        for subname in os.listdir(data_root):
            if os.path.isdir(os.path.join(data_root, subname)):
                for subfile in os.listdir(os.path.join(data_root, subname)):
                    if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-9][0-9][0-9].nii'):
...
if __name__ == "__main__":
    ...
    model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/train/UnetA_corr_gl_mBold_spatialM_bat_aal_Epoch_200.pt'
    model = unet_single_att_dsv_2D(in_channels=168, n_classes=2, feature_scale=1, is_batchnorm=False)
    ...
    with torch.no_grad():
        for X_ID, X, X_affine in test_loader:
            ...
            nib.save(img_output, '_'.join([X_dir, 'ML_UnetA_result.nii']))
            ...
            nib.save(bat_output, '_'.join([X_dir, 'ML_bat_UnetA_result.nii']))
    ...
    for subname in glob.glob('*/'):
        ...
        for subfile in os.listdir(os.path.join(test_dir, subname)):
            if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-9][0-9][0-9]_ML_UnetA_result.nii'):
            ...
            if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-9][0-9][0-9]_ML_bat_UnetA_result.nii'):
        ...
        img_output = nib.Nifti1Image(img_data_3D, img_mean.affine)
        img_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_aal_UnetA.nii'))

        img_bat_output = nib.Nifti1Image(img_bat_data_3D, img_mean.affine)
        img_bat_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_bat_output, os.path.join(test_dir, subname, 'ML_bat_result_spatialM_aal_UnetA.nii'))
