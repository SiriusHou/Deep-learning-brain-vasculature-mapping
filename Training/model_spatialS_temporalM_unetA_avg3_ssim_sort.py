...
class ChannelAttnUNet(nn.Module):
    def __init__(self, in_channels=50, n_classes=1, depth=5, wf=6, padding=False, batch_norm=False, up_mode='upconv'):
        ...
        self.Attn_block = ChannelAttnBlock(make_layers([64, 'M', 128, 'M'], in_channels=in_channels, batch_norm=True))
...
def make_layers(cfg, in_channels, batch_norm=False):
    ...
    layers += [nn.Conv2d(in_channels, 500, kernel_size=3, padding=1, stride=2, groups=in_channels_initial), nn.ReLU(inplace=True)]
    layers += [nn.MaxPool2d(kernel_size=2, stride=2)]
    layers += [nn.Conv2d(500, in_channels_initial, kernel_size=1, padding=0, stride=1, groups=in_channels_initial), nn.ReLU(inplace=True)]
    layers += [nn.MaxPool2d(kernel_size=2, stride=2)]
