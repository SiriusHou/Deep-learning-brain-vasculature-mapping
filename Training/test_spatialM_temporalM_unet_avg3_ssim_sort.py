...
if __name__ == "__main__":
    ...
    model_path = 'd:\\xhou4\\ML_CVR\\smooth8\\ML_data\\parameter_spatialM_temporalM_unet_sort_softAttn.pt'
    ...
    with torch.no_grad():
        for X_ID, X, X_affine in test_loader:
            ...
            h = np.squeeze(h.cpu().clone().numpy())
            np.savetxt('_'.join([X_dir, 'attention.txt']), h, fmt='%f')
            ...
            nib.save(img_output, '_'.join([X_dir, 'ML_softAttn_result.nii']))
    ...
    for subname in glob.glob('*\\'):
        ...
        for subfile in os.listdir(os.path.join(test_dir, subname)):
            if fnmatch.fnmatch(subfile, 'bold_spatialM_temporalM_sort_*_ML_softAttn_result.nii'):
        ...
        img_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_temporalM_unet_sort_softAttn.nii'))
