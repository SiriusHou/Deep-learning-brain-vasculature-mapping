            ...
            if os.path.isdir(os.path.join(data_root, subname)):
                for subfile in os.listdir(os.path.join(data_root, subname)):
                    if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-2]*'):
                        ...
                        # self.bat_IDs.append(os.path.join(parts_prefix, 'HC', 'CVR_HC_clean_re_' + parts_end))
...
if __name__ == "__main__":
    ...
    optim = AdaBelief(model.parameters(), lr=0.0001, eps=1e-10)
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_dual_corr_bold0_spatialM_aal_Epoch_100_valid_loss.png', dpi=300)
    plt.show()

    model_name = '/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_dual_corr_bold0_spatialM_aal_Epoch_100_AdaB_BAT.pt'
    if torch.cuda.device_count() > 1:
