...
class CVRDataset(data.Dataset):
            ...
            if os.path.isdir(os.path.join(data_root, subname)):
                for subfile in os.listdir(os.path.join(data_root, subname)):
                    if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-2]*'):
                        ...
                        self.bat_re_IDs.append(os.path.join(parts_prefix, 'bat', 'bat_HC_clean_re_' + parts_end))
    ...
    def __getitem__(self, index):
        ...
        D = torch.cat((B, B_re, C, C_re), dim=0)
        return [A, D]
...
if __name__ == "__main__":
    ...
    for t in range(epochs):
        ...
        if (t + 1) % 10 == 0:
            model_parts = ('/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_dual_corr_beta0_spatialM_aal_Epoch_', str(t+1) , '_AdaB1_1e12_new_re.pt')
