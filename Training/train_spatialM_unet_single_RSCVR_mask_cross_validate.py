                ...
                for subname in os.listdir(os.path.join(data_root, pod)):
                    if os.path.isdir(os.path.join(data_root, pod, subname)):
                        for subfile in os.listdir(os.path.join(data_root, pod, subname)):
                            if fnmatch.fnmatch(subfile, 'CVR_mean_temporal_f_clean_[0-2][0-9][0-9].nii') or fnmatch.fnmatch(subfile, 'CVR_mean_temporal_f_clean_[0-2][0-9][0-9][xyz].nii'):
    ...
    for pod in range(5):
        ...
        for t in range(epochs):
            ...
            if (t + 1) % 10 == 0:
                ...
                model_parts = ('/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate/', pod_test_name,'/Unet_single_RSCVR_corr_beta0_spatialM_ce_nm_Epoch_', str(t+1) , '_AdaB1_1e12_final2_mask_soft_4.pt')
