...
if __name__ == "__main__":
    ...
    test_dir = '/data1/xhou/ML_CVR/smooth8/ML_data/BrainTumor_OA/data/OA'
    ...
    for subname in glob.glob('*/'):
        ...
        img_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_ce_nm_Unet_dual_CVR.nii'))

        img_bat_output = nib.Nifti1Image(img_bat_data_3D, img_mean.affine)
        img_bat_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_bat_output, os.path.join(test_dir, subname, 'ML_result_spatialM_ce_nm_Unet_dual_BAT.nii'))
