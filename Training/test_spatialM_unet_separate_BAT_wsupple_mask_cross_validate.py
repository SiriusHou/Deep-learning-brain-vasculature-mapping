...
if __name__ == "__main__":
    ...
    for pod in range(5):
        ...
        model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate/' + pod_test_name + '/Unet_BAT_separate_wsupple_corr_beta0_spatialM_ce_nm_Epoch_100_AdaB1_1e12_mask_soft_4.pt'
        ...
        with torch.no_grad():
            for X_ID, X, X_affine in test_loader:
                ...
                bat = np.squeeze(prediction[0, 0].cpu().clone().numpy())
                ...
                nib.save(bat_output, '_'.join([X_dir, 'ML_Unet_separate_wsupple_result_BAT.nii']))
        ...
        for subname in glob.glob('*/'):
            ...
            for subfile in os.listdir(os.path.join(test_dir, subname)):
                if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_[0-9][0-9][0-9]_ML_Unet_separate_wsupple_result_BAT.nii'):
            ...
            nib.save(img_bat_output, os.path.join(test_dir, subname, 'ML_result_spatialM_ce_nm_Unet_wsupple_result_BAT4.nii'))
