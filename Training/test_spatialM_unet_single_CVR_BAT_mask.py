...
import math
from model_spatialM_unet_single_CVR_BAT import UNet_dual
...
if __name__ == "__main__":
    ...
    # test_dir = '/data1/xhou/ML_CVR/smooth8/ML_data/train_moyamoya_test'
    ...
    # test_dir = '/data1/xhou/ML_CVR/smooth8/ML_data/BrainTumor_OA/data'
    ...
    model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_single_corr_beta0_spatialM_ce_nm_Epoch_100_AdaB1_1e12_final2_mask_soft_4.pt'
    ...
    with torch.no_grad():
        for X_ID, X, X_affine in test_loader:
            ...
            nib.save(img_output, '_'.join([X_dir, 'ML_Unet_single_result_CVR.nii']))
            ...
            nib.save(bat_output, '_'.join([X_dir, 'ML_Unet_single_result_BAT.nii']))
    ...
    for subname in glob.glob('*/'):
        ...
        for subfile in os.listdir(os.path.join(test_dir, subname)):
            if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_[0-9][0-9][0-9]_ML_Unet_single_result_CVR.nii'):
            ...
            if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_[0-9][0-9][0-9]_ML_Unet_single_result_BAT.nii'):
                parts = subfile.split('_')
                ID = int(parts[-6]) - 1
        ...
        img_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_ce_nm_Unet_single_CVR4.nii'))

        img_bat_output = nib.Nifti1Image(img_bat_data_3D, img_mean.affine)
        img_bat_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_bat_output, os.path.join(test_dir, subname, 'ML_result_spatialM_ce_nm_Unet_single_BAT4.nii'))
