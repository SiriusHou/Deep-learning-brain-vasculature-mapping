...
import math
from model_spatialM_unet_re import UNet_re
...
class CVRDataset(data.Dataset):
        ...
        for subname in os.listdir(data_root):
            if os.path.isdir(os.path.join(data_root, subname)):
                for subfile in os.listdir(os.path.join(data_root, subname)):
                    if fnmatch.fnmatch(subfile, 'bold_spatialM_[0-2]*'):
                        ...
                        self.label_att_IDs.append(os.path.join(parts_prefix, 'HC', 'CVR_HC_clean_re_' + parts_end))
    ...
    def __getitem__(self, index):
        ...
        # Select attention label
        label_att_ID = self.label_att_IDs[index]
        #         print(label_ID)

        # Load label
        ta_att_img = nib.load(label_att_ID)
        label_att_image = ta_att_img.get_fdata()
        label_att_image = label_att_image.astype(np.float32)
        #         taimg.uncache()

        B_att = torch.Tensor(label_att_image).type(torch.FloatTensor)
        return [A, B, B_att]
...
if __name__ == "__main__":
    ...
    # Generators
    train_loader = data.DataLoader(training_set, batch_size=64, sampler=train_sampler, num_workers=6)
    validation_loader = data.DataLoader(training_set, batch_size=64, sampler=valid_sampler, num_workers=6)
    model = UNet_re(in_channels=11, n_classes=2, depth=5, batch_norm=False, padding=True, up_mode='upconv')
    ...
    for t in range(epochs):
        ...
        for i, (X, y, y_att) in enumerate(train_loader):
            X = X.to(device)  # [N, 6, H, W]
            y = y.to(device)  # [N, H, W] with class indices (0, 1)
            y = y.unsqueeze(1)

            y_att = y_att.to(device)  # [N, H, W] with class indices (0, 1)
            y_att = y_att.unsqueeze(1)
            y_all = torch.cat([y, y_att], 1)

            prediction = model(X)  # [N, H, W]
            loss = criterion(prediction, y_all)
            ...
            if (i + 1) % print_every == 0:
                ...
                model.eval()
                with torch.no_grad():
                    for (X, y, y_att) in validation_loader:
                        X = X.to(device)  # [N, 6, H, W]
                        y = y.to(device)  # [N, H, W] with class indices (0, 1)
                        y = y.unsqueeze(1)
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_re_corr_gl_spatialM_vtt_valid_loss.png', dpi=300)
    plt.show()

    model_name = '/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_re_corr_gl_spatialM_vtt.pt'
    if torch.cuda.device_count() > 1:
