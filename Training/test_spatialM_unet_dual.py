...
import math
from model_spatialM_unet_dual import UNet_dual
...
if __name__ == "__main__":
    ...
    model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_dual_corr_bold0_spatialM_aal_Epoch_100_AdaB_BAT.pt'
    ...
    with torch.no_grad():
        for X_ID, X, X_affine in test_loader:
            ...
            prediction = np.squeeze(prediction[0, 0].cpu().clone().numpy())
            ...
            # nib.save(img_output, '_'.join([X_dir, 'ML_Unet_dual_result.nii']))
            nib.save(img_output, '_'.join([X_dir, 'ML_Unet_dual_result_BATonly.nii']))
    img_size = (91, 109, 91)
    for subname in glob.glob('*/'):

        img_data_3D = np.zeros(img_size)
        for subfile in os.listdir(os.path.join(test_dir, subname)):
            # if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-9][0-9][0-9]_ML_Unet_dual_result.nii'):
            if fnmatch.fnmatch(subfile, 'bold_spatialM_aal_[0-9][0-9][0-9]_ML_Unet_dual_result_BATonly.nii'):
        ...
        img_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_aal_Unet_dual_BATonly.nii'))
