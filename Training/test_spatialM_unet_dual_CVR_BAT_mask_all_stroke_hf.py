    ...
    def __init__(self, data_root):
        ...
        for subname in os.listdir(data_root):
            if os.path.isdir(os.path.join(data_root, subname)):
                for subfolder in os.listdir(os.path.join(data_root, subname)):
                    if os.path.isdir(os.path.join(data_root, subname, subfolder)):
                        for subfile in os.listdir(os.path.join(data_root, subname, subfolder)):
                            if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_[0-9][0-9][0-9].nii'):
                                sub_filepath = os.path.join(data_root, subname, subfolder, subfile)
...
if __name__ == "__main__":
    ...
    test_dir = '/data1/xhou/ML_CVR/smooth8/ML_data/stroke_hf/data'
    os.chdir(test_dir)

    train_dir = '/data1/xhou/ML_CVR/smooth8/ML_data/train'
    ...
    model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate/Unet_dual_corr_beta0_spatialM_ce_nm_Epoch_100_AdaB1_1e12_final3_mask_soft_4.pt'
    ...
    for subname in glob.glob('*/'):
        ...
        for subfolder in os.listdir(os.path.join(test_dir, subname)):
            if os.path.isdir(os.path.join(test_dir, subname, subfolder)):
                ID = 0
                for subfile in os.listdir(os.path.join(test_dir, subname, subfolder)):
                    if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_[0-9][0-9][0-9]_ML_Unet_dual_result_CVR.nii'):
                        parts = subfile.split('_')
                        ID = int(parts[-6]) - 1

                        sub_filepath = os.path.join(test_dir, subname, subfolder, subfile)
                ...
                if ID != 0:
                    ...
                    img_output.get_data_dtype() == np.dtype(np.int16)
                    nib.save(img_output, os.path.join(test_dir, subname, subfolder, 'ML_result_spatialM_ce_nm_Unet_dual_CVR4.nii'))

                    img_bat_output = nib.Nifti1Image(img_bat_data_3D, img_mean.affine)
                    img_bat_output.get_data_dtype() == np.dtype(np.int16)
                    nib.save(img_bat_output, os.path.join(test_dir, subname, subfolder, 'ML_result_spatialM_ce_nm_Unet_dual_BAT4.nii'))
