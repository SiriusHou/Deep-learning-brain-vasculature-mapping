...
import math
from model_spatialM_unet_seperate_CVR_wsupple import UNet_separate_CVR
...
if __name__ == "__main__":
    ...
    for pod in range(5):
        ...
        model = UNet_separate_CVR(in_channels=136, n_classes=1, depth=5, batch_norm=True, padding=True, up_mode='upconv')
        ...
        for t in range(epochs):
            ...
            for i, (X, y) in enumerate(train_loader):
                ...
                if mask_index == 0:
                    print('Warning!')
                loss = criterion(prediction, y[:, 0, :, :].unsqueeze(1)) + 4*torch.sum(torch.abs( (prediction- y[:, 0, :, :].unsqueeze(1)) * y[:, 2, :, :].unsqueeze(1)))/(torch.sum(y[:, 2, :, :].unsqueeze(1))+0.00001)
            ...
            if (t + 1) % 10 == 0:
                ...
                model_parts = ('/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate/', pod_test_name,'/Unet_CVR_separate_wsupple_corr_beta0_spatialM_ce_nm_Epoch_', str(t+1) , '_AdaB1_1e12_mask_soft_4.pt')
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig('/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_CVR_separate_wsupple_corr_bold0_spatialM_ce_nm_Epoch_100_valid_loss.png', dpi=300)
        plt.show()
