...
if __name__ == "__main__":
    ...
    model_path = '/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_re_corr_gl_spatialM_vtt.pt'
    model = UNet(in_channels=11, n_classes=2, depth=5, batch_norm=False, padding=True, up_mode='upconv').to(device)
    ...
    with torch.no_grad():
        for X_ID, X, X_affine in test_loader:
            ...
            attention = np.squeeze(prediction[:, 1, :, :].cpu().clone().numpy())
            ...
            att_output = nib.Nifti1Image(attention, X_affine)
            nib.save(att_output, '_'.join([X_dir, 'ML_Unet_re_attention.nii']))
    ...
    for subname in glob.glob('*/'):
        ...
        for subfile in os.listdir(os.path.join(test_dir, subname)):
            if fnmatch.fnmatch(subfile, 'bold_spatialM_[0-9][0-9][0-9]_ML_Unet_re_result.nii'):
        ...
        img_output.get_data_dtype() == np.dtype(np.int16)
        nib.save(img_output, os.path.join(test_dir, subname, 'ML_result_spatialM_Unet_re.nii'))
