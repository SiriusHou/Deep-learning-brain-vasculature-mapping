...
import math
from model_spatialM_unet_single_BAT_only import UNet
...
class CVRDataset(data.Dataset):
    ...
    def __init__(self, data_root, test_pod):
        'initialization'
        self.input_IDs = []
        self.label_IDs = []
        self.label_re_IDs = []
        ...
        for pod in os.listdir(data_root):
            if os.path.isdir(os.path.join(data_root, pod)) and pod != test_pod:
                for subname in os.listdir(os.path.join(data_root, pod)):
                    if os.path.isdir(os.path.join(data_root, pod, subname)):
                        for subfile in os.listdir(os.path.join(data_root, pod, subname)):
                            if fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_single_[0-2][0-9][0-9].nii') or fnmatch.fnmatch(subfile, 'bold_spatialM_ce_nm_single_[0-2][0-9][0-9][xyz].nii'):
    ...
    def __getitem__(self, index):
        ...
        # D = torch.cat((B, B_re), dim=0)
        D = torch.cat((C, C_re), dim=0)
...
if __name__ == "__main__":
    ...
    for pod in range(5):
        ...
        training_set = CVRDataset('/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate', pod_test_name)
        ...
        model = UNet(in_channels=1, n_classes=1, depth=5, batch_norm=True, padding=True, up_mode='upconv')
        ...
        for t in range(epochs):
            ...
            for i, (X, y) in enumerate(train_loader):
                ...
                loss = criterion(prediction, y[:, :-1, :, :]) + 4*torch.sum(torch.abs( (prediction- y[:, :-1, :, :]) * y[:, 1:, :, :]))/(torch.sum(y[:, 1:, :, :])+0.00001)
                ...
                if (i + 1) % print_every == 0:
                    ...
                    with torch.no_grad():
                        for X, y in validation_loader:
                            ...
                            #                                                           y[:, :-2, :, :] * y[:, 2:, :, :])
                            loss = criterion(prediction, y[:, :-1, :, :]) + 4*torch.sum(
                                torch.abs( (prediction - y[:, :-1, :, :]) * y[:, 1:, :, :]) ) / (torch.sum(y[:, 1:, :, :])+0.00001)
            ...
            if (t + 1) % 10 == 0:
                ...
                model_parts = ('/data1/xhou/ML_CVR/smooth8/ML_data/cross-validate/', pod_test_name,'/Unet_single_BAT_corr_beta0_spatialM_ce_nm_Epoch_', str(t+1) , '_AdaB1_1e12_final2_mask_soft_4.pt')
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig('/data1/xhou/ML_CVR/smooth8/ML_data/train/Unet_single_BAT_corr_bold0_spatialM_ce_nm_Epoch_100_valid_loss_CVR_BAT.png', dpi=300)
        plt.show()
